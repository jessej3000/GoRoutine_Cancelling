...
func processRequest(w http.ResponseWriter, r *http.Request) {
	...
	if len(paramValues) > 0 { // If parameters exist then loop through it and make a request
		...
		timeout := time.After(500 * time.Millisecond) // set deadeline to 500 miliseconds before the for loop
		for {
			select {
				...
				if err != nil {
					...
					for counter := 0; counter < len(arrayNums); counter++ {
						if isItemInSlice(arrayNums[counter], combinedNumbers) == false {
							combinedNumbers = append(combinedNumbers, []int{arrayNums[counter]}...) // Combine all unique integers into one integer slice
						}
					}
				}
			case <-timeout: // Deadline comes here and cancel remaining go routines requesting from urls
				fmt.Println("Time Up")
				sort.Slice(combinedNumbers, func(i, j int) bool { // Sort slice
